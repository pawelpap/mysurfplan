...
**13 September mailbox setup:** Zoho Mail EU now hosts `pawel@mywaveplan.com`, with `support@mywaveplan.com` as an alias. MX, combined Zoho/Mailjet SPF, both DKIM selectors and monitoring DMARC are published in Vercel DNS. External delivery/replies and Mac-client sending are verified. The unused home.pl entry was removed; existing Growthmatics services are unchanged. [Current mailbox setup](MAILBOX_SETUP.md).

GoDaddy holds domain registration; Vercel holds authoritative DNS and hosts both app environments. Zoho hosts business/support mail. Mailjet remains the planned automated sender. The owner activated Mailjet integrations for both `mysurfplan-staging` and `mywaveplan-prod`; production-domain validation and SPF/DKIM passed on 10 September. The 13 September environment audit found no Mailjet API credentials in either live project. Credential/recipient isolation, account entitlements and real application delivery remain unverified. Use the current combined SPF, MX and DMARC in [MAILBOX_SETUP.md](MAILBOX_SETUP.md), not the earlier single-provider DNS state. The application adapter/jobs are now deployed disabled; complete the staging sender, credentials and activation tests in A6/A7. No nameserver move is required. [Email setup contract](AUTH_EMAIL_AND_JOBS_DECISION.md#confirmed-service-setup-10-september-2026).
