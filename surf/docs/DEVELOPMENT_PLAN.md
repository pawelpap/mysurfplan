...
- [x] A3: complete the [authentication/email/jobs decision](AUTH_EMAIL_AND_JOBS_DECISION.md). Retain accounts/sessions; use Mailjet Free for the initial pilot, a Neon outbox and a Pro minute worker. The owner selected `support@mywaveplan.com`. The owner has created the Mailjet account, added `mywaveplan.com` as its sending domain and activated the Vercel integration for both projects (owner-reported, not independently verified). Plan the receiving mailbox on existing home.pl Hosting Business, keep app hosting and DNS at Vercel, and retain domain registration at GoDaddy. Separately authorised domain activation completed on 10 September: Mailjet validates `mywaveplan.com` and SPF/DKIM both pass against existing DNS records. No agent DNS edit or app deployment was needed. Integration credential isolation, account limits, staging sender and delivery remain untested. Remaining mailbox, DMARC and application work stays in A6/A7/B3. AWS signup is not needed. SES, Scaleway and MailerSend remain assessed alternatives.
