...
**Next bounded task: A2, the supported dependency baseline.** A1's school boundaries, private response filtering, public schedule payloads, demo isolation and removal of `/test/*` pages are complete on staging and production. [Release evidence](archive/releases/RELEASE_2026-09-09_SCHOOL_ACCESS_PRIVACY.md) records the 81 regression tests, build and 12 real API/database scenarios per environment.

Review the Next.js 14.2.3 dependency tree and current advisories, choose a supported compatible version and keep the upgrade separate from membership or design changes. Acceptance: dependency/version decision, successful regression/build checks and affected authentication, access, forecast, public-schedule and booking journeys on staging, then an authorised production release. No router rewrite unless technically required.
