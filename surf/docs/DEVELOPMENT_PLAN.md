...
The forecasts and visual direction are now a product baseline the owner is happy with. Preserve them during the next membership and registration work. The owner also requested a future [conditions UX review based on Piotr's feedback](CONDITIONS_UX_REVIEW_PROPOSAL.md): calendar weeks, clearer day summaries, spot comparison cards, icons and a conditions chart. F17 requires Figma design and owner review before F18 implementation; this is not immediate UI work and does not change the forecasting model.
...
- [x] Owner accepted the registration model and permission matrix, 8 September: [accepted design](REGISTRATION_AND_MEMBERSHIP_PROPOSAL.md). B1's additive foundation is deployed; B2–B5 implement live global identity, registration and school flows.
...
- [ ] B2: use the new model in live session/API/UI authorisation. Support no-school accounts, several memberships and administrator/instructor combinations. Retire the bridge and replace single-school constraints safely.
...
- [ ] F17/F18: design and review the conditions-page improvements in Figma, then implement only the approved scope. Preserve all forecast parameters, meaningful experience levels, colour coding and mobile parity. Spot cards need bounded cached summaries; photos are optional. The [design brief](CONDITIONS_UX_REVIEW_PROPOSAL.md) records Piotr's suggestions and the assessment. This is conditional work that may be scheduled after B1/B2 and coordinated with C7; it does not add a launch gate.
...
**Next task: B2, global identity and school capability authorisation.** A1/A2/B1 are released, and the [A3 architecture decision](AUTH_EMAIL_AND_JOBS_DECISION.md) is complete. A3 selects Mailjet Free for the initial pilot, with SES, Scaleway and MailerSend retained as alternatives. Authentication stays application-owned; delivery uses the generic outbox contract. Email provider activation and lifecycle features are not implemented.

B2 starts by rechecking B1 reconciliation, then introduces the capability resolver and controlled authority switch. Preserve IDs, bookings, session revocation and current school boundaries. Retire the legacy bridge before independent membership writes; remove school-admin control of global credentials/status and the old school-to-user cascade at the verified transition point. Mailjet and the support mailbox are needed before B3 integration; no AWS signup is required for this plan.

A4/A5, capacity/recovery and EU-location decisions remain open. Localisation remains plan-only until authorised. The latest runtime verification and its limits are recorded in the [handover](HANDOVER.md) and [A2 release](archive/releases/RELEASE_2026-09-09_DEPENDENCY_BASELINE.md).
