...
17 September production release: the owner reviewed staging and explicitly approved promotion. Small-surf calibration across all 17 spots, regional Cornélia/São João settings, consistent best daylight windows on spot/day tiles, the bounded forecast archive and the provisional São Pedro GFS exposure curve are deployed on both environments. [Production release and verification](archive/releases/RELEASE_2026-09-17_FORECAST_PRODUCTION.md). F9/F14/F19 remain partial/open; this does not establish measured accuracy or complete a launch gate. The next core sequence remains A4/B6 → A9 → B3.

13 September storage decision: the owner selected **Vercel Blob in Frankfurt (`fra1`)** for B7 profile photos and school logos/graphics, replacing the earlier R2 candidate. Plan separate staging/production stores, private originals/drafts, public published derivatives, upload quotas and lifecycle cleanup. Neon holds references and metadata only. Provisioning belongs to B7; no infrastructure or subscription is changed by this planning update. [Scope](IMPLEMENTATION_ROADMAP.md#school-branding-and-customisation), [budget](LAUNCH_COST_ESTIMATE.md#vercel-blob-media-budget).

13 September design follow-up: the Support email shell uses self-hosted Poppins with Arial fallbacks, verified and deployed on both environments. [Preview and release evidence](archive/releases/RELEASE_2026-09-13_EMAIL_TYPOGRAPHY.md). The website's own font-loading cleanup remains in B6; actual inbox delivery tests remain an email-activation prerequisite.

13 September implementation: the A6/A7 email/job foundation is deployed on staging and production, with matching additive schema and verified app regressions. Sending and minute scheduling remain off. The worker remains disabled until email readiness passes. Pro is recommended for continued deployed commercial work and required before minute scheduling; no upgrade is recorded. [Purchase timing](INFRASTRUCTURE_CAPACITY_AND_UPGRADES.md). Complete Mailjet credentials, staging isolation and real delivery tests before B3 activation. [Runbook](EMAIL_JOBS_RUNBOOK.md).
...
Next core development task after the completed forecast release: **A4/B6 legal pages and acceptance records**. Review the actual operator/provider/retention details, then implement versioned privacy/terms pages, terms acceptance and separate optional preferences before public signup. A9 localisation foundations precede B3 registration. Finish the deferred Mailjet delivery tests and Vercel Pro scheduler before activating registration. **C5 cookie consent precedes C6 GTM/GA4 tracking**; optional tracking stays off until both are verified. Search Console/basic SEO do not require analytics consent.

**A4 preparation, 12 September:** supplied operator details, minimal personal-data scope, [inventory](PRIVACY_DATA_INVENTORY.md) and [draft policy/contract text](PRIVACY_DOCUMENT_DRAFTS.md) are recorded for review, not publication. Adult-only initial pilot; [F11](CHILDREN_AND_GUARDIAN_BOOKINGS.md) covers later children’s lessons. A8 has subsequently completed. Zoho mailbox readiness is complete. Legal/provider and retention review, plus Mailjet application delivery, remain open before registration.
...
Reviewed and rewritten: 7 September 2026. Implementation status reconciled: 16 September 2026. Expansion roadmap reviewed: 9 September 2026.
...
Location and language scope: A8 has moved both databases and Vercel application functions to Frankfurt. Maintain separate provider/log/support location evidence. A9/B10 remain planned before G1: accessible language switcher, English, European Portuguese and Spanish, with French optional. No localisation implementation has been authorised yet.
...
The owner approved F17’s [Conditions Figma proposal](CONDITIONS_UX_FIGMA_REVIEW.md) and approved F18 on staging and production. The [implementation contract](CONDITIONS_UX_IMPLEMENTATION.md) records the final refinements: spot cards for now in daylight or the next sunrise at night, five-minute batched summary caching, smaller mobile carousel cards, production’s 10 September noon day tiles, linked surf/energy and tide detail charts with one time label, equal parameter weight and full mobile tile information. Production was promoted and verified on 10 September after the owner’s staging review. That 10 September UX release did not change calculations or calibrations; the separately approved 17 September release supersedes its tile timing and adds database calibration changes.
...
The Caparica/São Pedro comparison is closed: the separate investigation explained the differences and verified both environments without a justified code/calibration change; the owner confirmed resolution on 16 September. [Report and disposition](archive/calibration/2026-09-15-four-spots/README.md). F19 remains broader planned research and does not block onboarding.

A new 17 September São Pedro review concerns Saturday 19 September. Surfline and our GFS inputs differ in swell bearing, height and partition ordering. The owner authorised a provisional assumption-based calibration for both breaks: Bico/Bafureira in both environments now admit a broader sheltered GFS direction range, retaining raw bearings and distinct break rules. [Exact settings, replay, live verification and remaining uncertainty](archive/calibration/SAO_PEDRO_DIRECTION_REVIEW_2026-09-17.md). This does not reopen the earlier comparison or establish measured accuracy. The owner subsequently approved production; both environments now use these revisions. [Production release and verification](archive/releases/RELEASE_2026-09-17_FORECAST_PRODUCTION.md).

Historical baseline: on 16 September both live domains and GitHub deployment branches were verified at documentation commit `04d9c9a8ba15e06406ceae6101154cc391719a34`, containing runtime `e6cfee77ee0517950107d0f01a76b7e0f900373f` from the [13 September typography release](archive/releases/RELEASE_2026-09-13_EMAIL_TYPOGRAPHY.md). Both environments now contain the approved 17 September small-wave/window and São Pedro release described above. The [handover](HANDOVER.md) owns current deployment IDs and verification limits. B2 global accounts/memberships, F18 Conditions, the translation fix, temporary public demo, A8 Frankfurt migration and the disabled email foundation are delivered. Self-registration, localisation and published privacy/terms remain unimplemented.

Completed targeted calibration review, 10 September: Praia Grande and Praia Pequena are revision 4 in both databases. Generic period-suitability parameters temper short-period quality without changing wave heights, energy or experience limits. Live verification passed on both environments. The [review and evidence](archive/calibration/2026-09-10-grande-pequena/README.md) records remaining Surfline differences; F19 covers coastal weather sampling and residual quality assessment. This does not change the agreed onboarding sequence.

The separately authorised [neighbouring Sintra follow-up](archive/calibration/2026-09-10-sintra-neighbours/README.md) extended that period-quality adjustment to Adraga, Praia das Maçãs, Magoito and São Julião on 10 September. Their other coefficients remain local. Both environments passed verification; F19 remains open for the wider source/model investigation.

Completed data review and production promotion, 7 September: 16 local spot calibrations revised, Praia da Torre added and generic São Pedro removed. That release matched spot/configuration and disposable lesson data; the later B2/A8 cleanup removed operational fixtures. The current handover owns the latest recorded inventory. All calibration changes use existing database parameters. Model and live checks passed; the owner approved production after staging review. Exact settings, research and limitations are in [Local spot calibration review](archive/calibration/2026-09-07-local-review/README.md). Continue validating the provisional coefficients against local observations. Surfline session data work is deferred at the owner's request. A1’s tenant/privacy audit and A2’s dependency/security cleanup were completed on 9 September. A3’s [authentication, email and jobs decision](AUTH_EMAIL_AND_JOBS_DECISION.md) is complete as a planning task. B1/B2 are deployed and verified; F17 is approved; F18 is deployed and verified on staging and production. B3 account/email self-service remains pending. The owner’s B2 UI feedback is recorded in [account and lifecycle follow-up](ACCOUNT_AND_LIFECYCLE_UX_FOLLOWUP.md).
...
Conservative operating budget after A3: [€90–120/month before tax and payment fees](LAUNCH_COST_ESTIMATE.md), assuming both minute workers, outbound email and one support mailbox. The intended activation schedules production only, with staging off outside tests; measure that workload through A7. About €50 remains a minimal forecast-only scenario if suitable free tiers suffice; it does not cover the new registration service. A7 settles scheduling, database active-time and recovery costs before live jobs. One-time professional work, domain renewal and fiscal invoicing remain outside this estimate. Purchase timing stays L1 for commercial-only costs; required technical upgrades happen earlier when needed.
...
- [x] Add revocable sessions and current server-side account/role/school checks. Deployed and verified on staging and production: single/global logout, password/status invalidation, disabled/deleted-account rejection and live single-school permissions. Multi-school capability was subsequently delivered by B2 in Phase 1.
...
- [x] A3: complete the [authentication/email/jobs decision](AUTH_EMAIL_AND_JOBS_DECISION.md). Retain accounts/sessions; use Mailjet Free initially, a Neon outbox and a minute worker subject to verified Vercel plan/capacity. Mailjet domain validation and SPF/DKIM passed on 10 September; app credentials, staging isolation, limits and delivery still require A6/A7 tests. On 13 September, Zoho Mail Lite replaced the home.pl receiving-mail proposal. Pawel/Support delivery, replies, Mac setup and DMARC monitoring are verified. Vercel retains DNS/app hosting and GoDaddy retains registration. B3 still requires actual application email flows. AWS signup is not needed.
...
- [ ] A4/B6: confirm the owner's Portuguese self-employed operator details and processing roles; prepare and review the privacy notice, platform terms, school data-processing agreement, internal processing/retention inventory and provider register. Publish accessible legal pages and implement version/language evidence and optional preferences through B6; retention and rights controls remain B8/A6/A8. [Detailed deliverables](COMMERCIAL_AND_PRIVACY_ARCHITECTURE.md#a4b6-documentation-and-implementation-scope). Review minors, VAT and invoicing with Portuguese legal/accounting support. Business registration alone does not establish GDPR compliance.
...
- [ ] A6: establish backups/restore checks, redacted server and browser error monitoring and an incident/support process. Include central browser crash reports with release/browser context, privacy controls and tested alerts when securing operations and maintenance. [Browser reporting scope, added 12 September](IMPLEMENTATION_ROADMAP.md#a6-browser-error-reporting-note-12-september-2026). Use synthetic/anonymised staging data; do not routinely copy production personal data there.
- [ ] A7: complete operational capacity and lifecycle planning. Neon is Launch, Vercel remains Hobby; A8 left four EU branches with five-minute compute suspension and tested seven-day recovery history. Measure use and resolve minute-worker capacity before email rollout. The US$20 notification is not a cap. Commercial-only upgrades remain L1; no automatic plan purchase.
- [x] A8: migrate staging and production databases and Vercel functions to Frankfurt, test restore/session continuity and app behaviour, remove unused legacy tables and retire the old US project/Vercel resource. Completed 12 September with owner-approved same-day disposal. [A8 release evidence](archive/releases/RELEASE_2026-09-12_EU_MIGRATION.md). Ongoing processor/legal review remains A4 and operational jobs remain A6/B8.
...
- [x] B1: add `school_memberships`, independent `membership_roles`, platform assignments and owner/invitation storage. Backfill proven roles and retain source IDs, credentials, sessions and history. The B1 bridge was used during migration and retired by B2. [Reconciliation and schema](MEMBERSHIP_DATA_MODEL.md).
...
- [ ] B4/B5: resolve explicit instructor/customer claims and legacy ownership through verified flows. B2 cleanup removed the earlier unclaimed test-person records; Demo Surf School ownership remains unresolved. Do not merge people by matching names or silently claim a school.
...
- [ ] B5/B7: add richer owner/admin School settings with logo, cover/gallery graphics, public profile/contact content and a constrained accent colour. Store typed school-scoped branding in the database and images in Vercel Blob Frankfurt, separately for staging and production, with quotas and lifecycle cleanup. Draft/preview/publish settings feed both public pages and lesson emails. See the [school branding scope](IMPLEMENTATION_ROADMAP.md#school-branding-and-customisation).
...
- [ ] B7/B8: add photo uploads to the selected Vercel Blob stores, data export and account deletion. Transfer school ownership or close the school properly before deleting its sole owner.
...
- [ ] C2: build a branded public school page and bookable schedule, using shared school settings for logo, cover/gallery, description, contacts and limited section choices. Keep the MyWavePlan layout, booking journey and required disclosures coherent on desktop/mobile.
- [ ] C4: add attendance and assigned-instructor access, confirmations, cancellations, weather rescheduling, reminders and calendar links. Let school admins customise bounded email copy and branding with a preview, while the app owns booking facts, sender authentication and required notices. Reuse the shared email shell/outbox; registration/security emails remain platform-owned.
...
- [ ] F14, partial on staging: bounded immutable sampling is implemented on 17 September; fixed-lead coverage, monitoring and storage strategy remain open. Continue forecast archiving within A5's permitted evaluation/archive scope and technical limits. Commercial-only access is activated through L1 if that work requires it, not assumed necessary for every prototype. Record issued/provider/retrieved/valid timestamps and input/model/calibration versions; do not rebuild supposed historical predictions from newer data.
...
- [ ] F19: implement the owner's [accepted shared-model and weather-sampling review](IMPLEMENTATION_ROADMAP.md#f19-accepted-scope-10-september-2026). Audit all spot settings, validate configurable coastal weather sampling and review period/size/wind/wind-sea interactions across representative beaches. Keep local coefficients in the database; no universal period cut-off or copying Grande/Pequena's provisional weights across the catalogue. Validate against observations before wider rollout. The 17 September trial implements a bounded staging subset; the broader review and production approval remain open.
...
Keep one application with clear identity, booking, billing, catalogue, media and privacy modules. No microservice rewrite is needed. Use Neon for relational records/metadata, Vercel Blob in Frankfurt for images and Stripe for payment collection. Enforce permissions and entitlements on the server.
...
Proposed operator identity, supplied by the owner: **PAWEL PAPLINSKI, tax number PT311219217**. The owner also confirmed Portuguese self-employed status, address and the privacy contact on 12 September; retain unpublished private details in the ignored drafts. Complete legal/provider review and applicable VAT/invoicing decisions before publishing the relevant documents. This information alone does not establish compliance.
...
The remaining sequence is **A4/B6 reviewed policies and acceptance records → A9 localisation foundations → B3 registration, verification and recovery**. A8 is complete; do not prepare or repeat the EU migration. The A6/A7 outbox/worker and Zoho mailbox are delivered. Before registration activation, complete isolated Mailjet credentials/staging sender, real delivery/callback tests, independent alerts, Pro scheduling and measured Neon capacity. See the [roadmap sequence](IMPLEMENTATION_ROADMAP.md#agreed-next-sequence-10-september-2026) and [activation runbook](EMAIL_JOBS_RUNBOOK.md). A9 remains planned until its implementation is authorised. This reconciliation starts no feature, purchase or deployment.
