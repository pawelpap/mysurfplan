...
**Next bounded task: A3, the authentication, transactional email and background-job decision.** A1 access/privacy protections and A2 dependency cleanup are complete on staging and production. The latest release passed 83 regression tests, build/lint, 13 real API/database scenarios per environment and desktop/mobile browser checks, including nearest-spot selection before production. [Release evidence](archive/releases/RELEASE_2026-09-09_DEPENDENCY_BASELINE.md).

Compare retaining the current authentication with a managed option against verified signup, password recovery and platform-admin MFA requirements. Specify the email provider, sender/domain verification and durable retry/job approach. Record costs, processor/region coverage and operational requirements before provisioning. Avoid combining a provider rewrite with membership migration without a clear reason.

Then implement B1's additive global identity/membership schema and backfill, followed by B2's authorisation and workspace changes. Preserve IDs, bookings, session revocation, school boundaries and independent admin/instructor roles. A4/A5, capacity/recovery and EU-location decisions remain open. Localisation remains a documented future task; no implementation was requested yet.
