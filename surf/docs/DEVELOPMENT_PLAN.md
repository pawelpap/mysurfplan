...
13 September implementation: the A6/A7 email/job foundation is deployed on staging and production, with matching additive schema and verified app regressions. Sending and minute scheduling remain off. The owner will upgrade Vercel when the registration flow needs the worker. Complete Mailjet credentials, staging isolation and real delivery tests before B3 activation. [Runbook](EMAIL_JOBS_RUNBOOK.md).
...
**A4 preparation, 12 September:** supplied operator details, minimal personal-data scope, [inventory](PRIVACY_DATA_INVENTORY.md) and [draft policy/contract text](PRIVACY_DOCUMENT_DRAFTS.md) are recorded for review, not publication. Adult-only initial pilot; [F11](CHILDREN_AND_GUARDIAN_BOOKINGS.md) covers later children’s lessons. A8 has subsequently completed. Mailbox readiness, legal/provider and retention decisions remain open before registration.
...
The owner-approved sequence is A4 privacy/data inventory → A8 EU migration → A6/A7 email/job readiness → B3 account/email self-service. Prepare A8 alongside A4 and confirm A7 migration capacity/costs before provisioning. Rehearse staging before production with backup, restore and rollback checks. B6 policies and separately authorised A9 localisation remain B3 prerequisites. See the [agreed sequence](IMPLEMENTATION_ROADMAP.md#agreed-next-sequence-10-september-2026). This update schedules the work; it does not execute or authorise a migration. A3 selected Mailjet Free and a generic outbox contract; the Mailjet account, sending-domain entry and both Vercel integrations are owner-reported as set up. Production-domain validation and SPF/DKIM now pass. Staging sender, integration credentials and delivery still need verification; application email code remains pending; the Zoho receiving mailbox is verified. No AWS signup is needed. Localisation remains plan-only until authorised.
