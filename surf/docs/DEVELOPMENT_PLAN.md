...
**Next bounded task: B1, additive global membership schema and backfill.** A1/A2 are released, and the [A3 architecture decision](AUTH_EMAIL_AND_JOBS_DECISION.md) is complete. A3 now selects Mailjet Free for the initial pilot, with SES, Scaleway and MailerSend retained as alternatives. Authentication stays application-owned; delivery uses the generic outbox contract. Email provider activation and lifecycle features are not implemented.

Start with branch capacity, identity/ownership reconciliation and a reversible migration rehearsal. Preserve IDs, bookings, session revocation and current school boundaries. Do not infer ownership or mark legacy emails verified. B2 then changes live authorisation and supports independent admin/instructor roles. Mailjet and the support mailbox are needed before B3 integration, not before B1; no AWS signup is required for this plan.
