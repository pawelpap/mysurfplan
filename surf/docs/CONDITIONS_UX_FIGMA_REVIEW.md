...
- **Spot cards show now during daylight or the next sunrise at night**, following the owner’s 10 September refinement. Both the carousel and All spots use per-spot local time labels and remain independent of the calendar and chart cursor. Five-minute batched summaries replace the earlier per-minute requests; previous values remain visible during updates. The implementation contract records boundary and polar behaviour. Quality filtering must never imply that a green Advanced forecast is suitable for a beginner.
...
- Both graphs and detailed parameters share the selected time. The top spot comparisons remain on their now/next-sunrise context and do not change when a future date/hour is selected. Support tap/click, drag and keyboard/hour-list alternatives. Mobile must allow normal vertical scrolling without accidental time changes.
