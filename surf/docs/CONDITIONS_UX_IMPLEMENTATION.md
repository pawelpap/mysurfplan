...
## Tile-parameter follow-up, staging and production

The owner requested numerical wind/swell bearings on spot tiles and wind speed plus swell energy on both spot and day tiles. Spot tiles retain compass directions alongside degrees. The same shared metric rows show wind in km/h and existing offshore swell energy in kJ/m², including missing and partial-data states. Values use each tile's existing timestamp: now/next sunrise for spots, noon for days. The compact summary API includes wind speed and the existing energy assessment; no extra forecast call or formula change is needed. Keep two-card mobile width and equal visual emphasis. [Staging evidence](archive/releases/RELEASE_2026-09-10_TILE_PARAMETERS_STAGING.md) and [production evidence](archive/releases/RELEASE_2026-09-10_TILE_PARAMETERS_PRODUCTION.md) record the owner-approved rollout and verification.

The tile follow-up uses exactly two measurement rows: icon, direction arrow, numerical bearing, compass direction, then swell energy or wind speed. There are no separate Wind/Energy label rows. Weather sits at the top right beside the spot name or date. Day columns allow enough width for the full values on phones, retaining the weekly grid and scroll navigation.
