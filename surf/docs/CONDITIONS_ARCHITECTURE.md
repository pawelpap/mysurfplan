...
17 September release, approved and deployed on staging and production: all 17 spot size-ceiling curves permit favourable clean small surf; Cornélia and São João additionally trial directional spread 95° while retaining distinct normals/gains. General defaults are profile v2 in both environments. Spot and day tiles use the same best daylight-window selector. Fresh forecast inputs/configuration are sampled immutably per six-hour slot with 90-day opportunistic retention. The shared scoring formula is unchanged. [Production release and verification](archive/releases/RELEASE_2026-09-17_FORECAST_PRODUCTION.md). F9/F14/F19 remain partial/open; this does not establish measured accuracy.

Subsequent 17 September São Pedro trial, now approved and deployed in both environments: Bico revision 8 and Bafureira revision 7 retain their exposure through 283°, admit a sheltered GFS 285–300° shoulder and taper strongly to 310°. This replaces the earlier numerical cut-off in those two database profiles, without changing raw bearings, provider selection or the generic evaluator. The owner's original 283° guidance describes Surfline, whose bearings are not interchangeable with GFS. [Exact curve, assumption, checks and limitations](archive/calibration/SAO_PEDRO_DIRECTION_REVIEW_2026-09-17.md).
...
Opening or reloading either Conditions or a lesson requests a fresh forecast. Manual “Refresh” does the same and displays a disabled “Refreshing…” button while retaining the visible forecast. Healthy results fetched within two minutes are shared to limit duplicate upstream calls; incomplete results can be forced again after one minute.

While a screen remains open, it checks every five minutes when visible, and when focused or visible again after one minute. Following a failed refresh, it checks after the server's `retryAt`, on a 30-second timer. Normal shared cache expiry is 15 minutes. Incomplete results and failures have a one-minute retry back-off, extended when the provider supplies a longer `Retry-After`. Provider fetches use `no-store`; browser responses are private and not cached.

Marine, weather and water-temperature requests run in parallel. Each has a 12-second timeout and at most one retry for network failures, timeouts, rate limiting or server errors. Short retries wait 0.5–1 second. Longer provider delays are handled by the shared cache back-off rather than blocking the HTTP request. Permanent client errors are not retried immediately. Diagnostics record the source, attempt, HTTP status, timeout flag and duration, without request URLs, keys or raw response bodies. A 40-second database lease prevents duplicate refreshes across instances. Concurrent callers wait up to 30 seconds for that refresh, even if they already have an older forecast. Timestamp-guarded expiry avoids invalidating a refresh that completed after the initial cache read.

Provider cache version, wave-model identifier and sample coordinates must match the spot. Incompatible cached inputs are refreshed and cannot be scored under a different source configuration. A partial wave or weather response cannot replace a compatible complete forecast fetched less than 24 hours ago. The whole previous forecast is retained with its original fetch time, `stale: true` and an explicit warning; fresh waves are never combined with old wind or weather. Without a usable complete forecast, partial data remains explicitly unavailable where required inputs are missing and is cached only until the next retry. Beyond 24 hours, old data is unavailable. An optional water-temperature failure does not block fresh wave/weather data. There is no frozen forecast stored on a lesson: every request uses the current spot calibration and the latest shared provider data. Astronomical tides are recalculated from harmonic constants; they do not need a new weather-model run.
