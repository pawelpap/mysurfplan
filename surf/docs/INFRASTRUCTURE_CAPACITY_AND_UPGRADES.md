...
16 September reconciliation: the Vercel team is still Hobby, verified through the connected team API. **Pro is recommended for continued deployed commercial work and is required before testing/enabling the minute schedule.** The earlier owner deferral concerned technical worker timing; it does not override commercial-use requirements. No purchase is recorded or authorised by this update. The email foundation is already deployed; keep sending/scheduling off until provider delivery, isolation, budgets and independent alerts pass. [Runbook](EMAIL_JOBS_RUNBOOK.md).
...
[A8 completed runbook](EU_DATA_MIGRATION_PLAN.md) records database/runtime placement, legacy cleanup, checks and US retirement. Metered migration overlap was temporary; actual invoice usage was not measured here. Zoho mailbox readiness is complete. Mailjet application delivery, minute-worker capacity and alerts remain A6/A7 prerequisites before registration.
...
Commercial-only purchases are scheduled immediately before the first use that requires them, with time to configure and test. Early work checks permitted use and technical limits without activating subscriptions unnecessarily. Capacity, security and recovery requirements remain reasons to upgrade earlier. For MyWavePlan, the current recommendation above is Pro for continued deployed commercial work. Neon Launch is already active; do not repeat its purchase.

The [monthly estimate](LAUNCH_COST_ESTIMATE.md) retains approximately €90–120/month for the conservative two-continuously-polled-endpoints scenario, outbound email and the already-active mailbox. The intended activation schedules production only, with staging off outside tests. The earlier €50 minimal forecast configuration assumes suitable free tiers and excludes those new flows. Database continuous active time is now explicit. This is a future scenario, not measured billing or approval to purchase.
...
Neon Launch is already active and A8 rehearsed seven-day recovery. A6/A7 must verify ongoing recovery operations, capacity, branch lifecycle and metered cost before real bookings. The earlier Free-versus-Launch decision is historical; no downgrade or second purchase is planned.
...
Retain the existing Launch setup and measure its workload after sensible branch/data lifecycle management. Revisit Scale only when its restore/security/support capabilities or measured compute ceiling are needed. Do not treat a website's typical-spend example as a fixed subscription quote, and do not promise a total monthly bill from the current small database size.
...
Mailjet Free is the selected initial-pilot application sender. The existing account and production domain are set up; SPF/DKIM validation passed. The outbox/worker/adapter is deployed disabled. Zoho Mail Lite receives business and Support mail, with delivery/reply tests complete. Before B3 activation, configure isolated Mailjet credentials and staging sender, then verify real delivery, callbacks, quotas and replies to Zoho. A4 reviews agreements and retention; A8 primary placement is complete. No AWS setup or replacement mailbox is needed. [Activation runbook](EMAIL_JOBS_RUNBOOK.md), [mailbox evidence](MAILBOX_SETUP.md).
