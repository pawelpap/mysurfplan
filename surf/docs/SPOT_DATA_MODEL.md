...
Document role: maintained schema reference. Numerical examples and linked release records are dated evidence; current coefficients come from the database and its calibration history. See the [latest documented spot rollout](archive/calibration/2026-09-07-local-review/README.md) and [current handover](HANDOVER.md).
...
The [São Pedro adjustment of 6 September](archive/calibration/CALIBRATION_2026-09-06_SAO_PEDRO.md) demonstrates a later calibration using this schema: only `exposureByDirection` and `sizeCeilingCurve` were edited in the three spot records, with no engine or page-code change. Their different tide and experience rules were preserved. Current calibration lives in the database; historical seeds and test fixtures are not a substitute for the latest saved revision.
