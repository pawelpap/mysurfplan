...
| Receiving support mailbox | `support@mywaveplan.com`; service not yet configured | Reserve **€2** for one small mailbox | Provider/location to approve in A4. Zoho Mail Lite publishes a US$1/user/month example for 5 GB, billed annually. Mailjet outgoing delivery does not create this inbox. |
...
A5 confirms source/service use and L1 activates required commercial access just before its triggering activity. A3’s [decision](AUTH_EMAIL_AND_JOBS_DECISION.md) retains auth, selects Mailjet Free initially and specifies the outbox/worker. A7 verifies current Vercel/Neon usage, scheduling, branch lifecycle and recovery before activation; A4 settles the receiving mailbox and processor review; B7 chooses media storage. Compare measured pilot usage with these scenarios before approving spend, then update after the first billing period and new payment/archive workloads.

No subscription, payment, account connection, application code or database was changed for A3. Mailjet replaces the initial SES proposal; the owner-selected `support@mywaveplan.com` still needs a receiving mailbox. No AWS signup is required. Account/mailbox activation can wait until B3 integration preparation. The next development task is **B1: additive global membership schema and backfill**.
