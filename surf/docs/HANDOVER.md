...
Staging and production contain reviewed release `26e2a17ca9d524bb090fbe651662b6d365a2b164`, including application runtime `3032204543a11308766efe148699624466d66bb3`. Small-surf calibration, consistent selected-day daylight windows, the bounded forecast archive and both São Pedro direction profiles are deployed. The active development branch is `codex/small-wave-windows`; subsequent release-record commits do not change application runtime. [Production release and verification](archive/releases/RELEASE_2026-09-17_FORECAST_PRODUCTION.md).

**Production approval received and rollout verified, 17 September.** The owner confirmed staging looked correct and instructed its deployment. This supersedes the earlier hold for this release. Both calibration migrations and the archive schema are applied to production; future releases still need their own authority.

| Environment | URL | Git branch | Vercel project | Reviewed runtime deployment |
| --- | --- | --- | --- | --- |
| Staging | [staging.mywaveplan.com](https://staging.mywaveplan.com) | `staging` | `mysurfplan-staging` | `dpl_HgyNkngQNx4QHupvBfaQTBWwSNza` |
| Production | [mywaveplan.com](https://mywaveplan.com) | `main` | `mywaveplan-prod` | `dpl_BL3VeqBzsef2vabxafFRf2f9aQWG` |

Both projects use `fra1` functions and Node.js 22. All 137 tests pass. Production verified six complete 16-day forecasts, hourly scoring, exact top-card/day-window parity for 17/19/20 September, anonymous/demo permissions, all 17 configuration revisions, archive samples and desktop/mobile behaviour. [Production release and verification](archive/releases/RELEASE_2026-09-17_FORECAST_PRODUCTION.md) records evidence and limits. One existing avatar-image lint warning remains for B7. These checks do not establish measured forecast accuracy.
...
**The forecast release is complete.** The next core development task is **A4/B6 legal pages and acceptance records**, followed by A9 and B3 with the email activation prerequisites below.
...
The new 17 September Cornélia observation is separate from the closed comparison above. The owner reported clean 0.6–0.9 m surf at 08:00–09:00. Both environments now relax the small-surf size ceiling across all 17 spots and trial wider directional spread at Cornélia and São João. Both Caparica spots are revision 6; the general default profile is v2. Other spots advance one revision for small surf, with the additional São Pedro revisions below. [Trial, source limitations and review](archive/releases/RELEASE_2026-09-17_SMALL_WAVE_STAGING.md).

F9/F14/F19 remain open/partial. One structured observation fixture and six-hourly request-driven immutable forecast samples with 90-day retention are now deployed on staging and production. This is not complete observation entry, fixed-lead archive coverage or an accuracy dashboard. No historical forecasts were invented from later retrievals. No cron was added.

Subsequent 17 September São Pedro trial: the owner authorised a provisional assumption after identifying Saturday's GFS/Surfline direction and component-height disagreement. Bico is revision **8** and Bafureira **7** in both environments. Their database exposure curves admit sheltered GFS 285–300° swell and taper strongly to 310°; displayed raw bearings and break-specific rules are unchanged. Saturday's live best windows are both 08:00–09:00, Fair, about 0.3–0.5 m; Sunday remains flat. All 137 tests and live API/browser checks passed. [Exact assumption, full replay, limitations and rollback](archive/calibration/SAO_PEDRO_DIRECTION_REVIEW_2026-09-17.md). This is separate from the closed 15 September comparison. The owner approved production after reviewing both changes; the two migrations were promoted atomically with preserved history.
