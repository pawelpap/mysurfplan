...
13 September A6 implementation adds `job_environment`, `job_outbox`, `email_suppressions`, `email_event_receipts`, `email_usage` and `job_worker_status`. Sending is off and no real email payload is enqueued. Future delivery stores encrypted short-lived recipients, HMAC suppression hashes and minimal event/status metadata. Proposed payload/30-day metadata retention, suppression retention, Mailjet-created contacts and encrypted backup/key handling still require A4/B8 review before real email. [Runbook](EMAIL_JOBS_RUNBOOK.md).
...
| Business email | pawel@mywaveplan.com, Zoho Mail Lite active and delivery/replies verified 13 September |
| Support and privacy email | support@mywaveplan.com, selected by the owner; Zoho delivery/replies verified 13 September |
| Responsible person | Pawel Paplinski, operator and initial privacy-request owner; no DPO appointment is implied |

Use the supplied identity in drafts. This audit has not checked a business registration certificate, VAT registration/exemption, business activity codes or invoicing obligations. Do not describe the operator as a separate incorporated company. Zoho Support delivery/reply tests passed on 13 September. Review the policy wording and monitoring process before publication; [mailbox status](MAILBOX_SETUP.md).
...
| P11 Business/support correspondence | Operator contact/contract details with providers and user support through the active Zoho mailbox/Support alias; earlier Proton correspondence is historical | Operator, recipients and their mail providers | Operator controller for business correspondence; contracts/legal obligations or assessed legitimate interest as applicable | Zoho mailbox is active and the operator is the initial request owner. A4/A6 must finalise correspondence retention, response process and secure handling |
...
17 September staging addition: `spot_forecast_snapshots` stores environmental forecast inputs, spot calibration and tide data without visitor/session identifiers. Sampling is request-driven, at most once per six-hour slot/spot/engine/calibration. Retention is 90 days; cleanup removes at most 100 expired rows on later successful refreshes, so this is not a guaranteed purge deadline. The owner's one surf-session report is a provenance-labelled development fixture, not a new public observation or tracking flow. Full observation privacy controls remain F9; provider/archive-use review remains A5. Production has neither the snapshot table nor the trial calibration yet.
...
| Zoho Mail EU, active 13 September | Pawel mailbox and Support alias; business/support messages and attachments; IMAP on the owner’s Mac | EU account/endpoints and delivery verified. Review contract/DPA, backup/support processing locations, retention and export/deletion; client devices also hold synchronised mail |
