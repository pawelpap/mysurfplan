...
Energy and water temperature use the same label, value, note and spacing styles as the other forecast parameters. They appear inside the shared conditions grid, with no separate card background, icon or larger type. The selected-time summary includes quality and experience in the same grid and uses four columns on desktop and two on smaller screens; lessons use four columns on desktop and two on smaller screens. Expanded desktop and mobile hours share the same metric cells and swell-component details. An optional explanation contains the interpretation and limits. Desktop hourly details open from the time/chevron; mobile details open from the hour summary. Both expose energy, power, water temperature and all available swell components. Lessons explicitly label both values as at start.

The owner-approved presentation release of 6 September 2026 adds shared light/dark styles and restores semantic surf-quality colours without giving energy or water additional emphasis. It is deployed on production and staging. See [release notes](archive/releases/RELEASE_2026-09-06_PRESENTATION.md).
