...
Two quiet teal cards sit together below the existing conditions metrics. Energy uses kJ/m² with estimated power beneath; water uses °C. The cards use the existing colours, type and spacing, with two columns on phones and one below 360 px. An optional explanation contains the interpretation and limits, keeping the main screen concise. Hourly desktop rows include energy beneath swell and water beneath weather; expanded mobile hours use compact cards. Lessons explicitly label both values as at start.
