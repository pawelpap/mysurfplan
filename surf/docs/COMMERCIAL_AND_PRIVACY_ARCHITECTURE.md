...
Status: the identity/membership design was accepted on 8 September; A3’s authentication/email/jobs direction was documented on 9 September. B1/B2 membership runtime, A8 placement and the disabled email foundation are implemented; commercial/policy choices and remaining provider activation are pending. The domain tables below mix existing and proposed records; the roadmap and runtime references identify what is shipped. Confirm provider choices, policies and responsibilities before implementation. This document is neither a published privacy notice nor confirmation of legal compliance.
...
The [A3 decision](AUTH_EMAIL_AND_JOBS_DECISION.md) retains current authentication, with lifecycle/password hardening and MFA still to implement. Mailjet Free is selected for initial transactional email; the Neon outbox, Vercel worker and adapter are deployed with sending/scheduling disabled. Zoho Mail Lite and the Support alias are active and verified. The Mailjet production domain passes SPF/DKIM, but isolated app credentials, the staging sender and real delivery/callback tests remain open. A8 placement is complete; A4 reviews agreements, wider processor handling and retention before activation. See the [email runbook](EMAIL_JOBS_RUNBOOK.md) and [mailbox record](MAILBOX_SETUP.md).
...
Owner decision, 13 September: use **Vercel Blob in Frankfurt (`fra1`)** for B7 images. This replaces the earlier Cloudflare R2 candidate; storage is selected but not provisioned. Create separate staging/production stores with separately scoped credentials, and separate private originals/drafts from public published derivatives. Keep asset IDs and object references behind the media module so business records do not depend on a provider-specific URL. Existing Vercel DNS remains in place. [Region selection](https://vercel.com/docs/vercel-blob#choosing-your-blob-store-region), [B7 scope](IMPLEMENTATION_ROADMAP.md#school-branding-and-customisation).

A4/B7 must check Vercel's applicable DPA, subprocessors, access controls and deletion/cache behaviour before enabling uploads. Frankfurt identifies the selected storage region; public delivery uses a global CDN and does not establish that all processing or copies stay within the EU. Disclosures must describe the verified setup. Budget storage, requests, downloads, cache-miss transfer and any image processing together; use quotas and usage alerts. [Cost assumptions](LAUNCH_COST_ESTIMATE.md#vercel-blob-media-budget). Future F14 archives require a separate private-storage/retention cost estimate before activation.

Use the supported Vercel Blob upload-token and private-download mechanisms, with short-lived authorisation, server-side ownership checks, upload quotas and size/dimension limits. Verify permissions before issuing upload access; verify completed content before marking an asset usable or publishing derivatives. Decode/re-encode images, strip EXIF/location metadata and reject unsupported/active content. Generate appropriately sized thumbnails; do not serve originals in small cards. Keep originals private. Publish derivatives only for an explicitly public profile or school page and purge them when visibility is withdrawn. Support mobile camera formats through a defined conversion path; test orientation and slow uploads.
