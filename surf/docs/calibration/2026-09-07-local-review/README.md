...
Applied to **staging only**, at 19:45 UTC. Production approval is pending. This is a database change using the existing generic schema v3; no application code, shared default profile, provider integration or Vercel deployment changed.
...
- Read-only production fingerprints before/after matched for every spot's version, configuration and active status. Production retains generic São Pedro and does not yet include Torre.
...
For a future approved production rollout, re-read current versions, map spots by slug, validate every configuration and update each spot with an optimistic version guard and history entry. Do not copy the whole staging database or overwrite users/lessons. Confirm the scope of production generic-spot lesson removal against the owner's approval at that time. A rollback restores selected previous configurations as new revisions with history notes. Recreating deleted generic São Pedro is possible from its saved row/history; its disposable test lessons are intentionally not recoverable from this evidence bundle.
