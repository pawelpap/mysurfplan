...
A8 is complete. Subsequent work delivered the Zoho mailbox and disabled A6/A7 email/job foundation. The current next task is A4/B6 policies and acceptance, followed by A9 localisation and B3 registration. Mailjet delivery/isolation tests, alerts, scheduling and capacity checks remain prerequisites for registration activation. Follow the [current handover](HANDOVER.md) and [email runbook](EMAIL_JOBS_RUNBOOK.md); do not repeat the migration or old mailbox setup.