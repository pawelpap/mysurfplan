...
test("school context comes from the session; supplied filters cannot grant access", () => {
  ...
  for (const value of [
    ...
    ["school-a"],
    {},
    "a15914fc-0cdb-4604-804d-322c8d6dedb6",
  ])
...
async function loadHandler(path, sql, role = "student") {
  ...
  globalThis[key] = {
    ...
    requireAuth: async () => ({
      ...
      schoolId,
      capabilities: {
        manageSchool: ["school_admin", "platform_admin"].includes(role),
      },
    }),
    requireMutation: () => true,
  };
  let source = await fs.readFile(
    new URL("../pages/api/" + path, import.meta.url),
    "utf8",
  );
  source = source
    .replace(
      /import \{ sql \} from [^;]+;/,
      `const { sql } = globalThis.${key};`,
    )
    .replace(
      /import \{ requireAuth \} from [^;]+;/,
      `const { requireAuth } = globalThis.${key};`,
    )
    .replace(
      /import \{ requireMutation \} from [^;]+;/,
      `const { requireMutation } = globalThis.${key};`,
    )
    .replace(
      /(['"])(?:\.\.\/)+lib\/school-access\.mjs\1/g,
      JSON.stringify(new URL("../lib/school-access.mjs", import.meta.url).href),
    );
...
test("school edits bind arbitrary text as values through the Neon query API", async () => {
  ...
  const sql = async () => {
    throw new Error("Expected the parameterised query API");
  };
  sql.query = async (text, values) => {
  ...
  await handler(
    {
      method: "PATCH",
      query: { id: schoolId },
      body: { name, contactEmail: "school@example.invalid" },
    },
    res,
  );
  assert.equal(res.statusCode, 200);
...
test("student and instructor directory responses omit private instructor fields", async () => {
  for (const role of ["student", "coach", "school_admin", "platform_admin"]) {
    ...
    assert.deepEqual(
      res.body.data,
      ["student", "coach"].includes(role)
        ? [{ id: schoolId, name: "Instructor" }]
        : [row],
    );
  }
});
...
test("public lessons validate filters before querying and retain valid level filters", async () => {
  let calls = 0;
  const handler = await loadHandler("public/lessons.js", async () => {
    calls++;
    return [];
  });
  for (const query of [
    ...
    const res = response();
    await handler(
      { method: "GET", query: { school: "school-a", ...query } },
      res,
    );
  ...
  await handler(
    {
      method: "GET",
      query: { school: "school-a", difficulty: "Beginner", from: "2026-09-01" },
    },
    res,
  );
...
test("database failures never expose database messages or details in public or directory responses", async () => {
  const error = Object.assign(
    new Error("connection string and SQL contain private@example.invalid"),
  ...
  for (const path of [
    ...
    const handler = await loadHandler(path, async () => {
      throw error;
    });
    const res = response();
    await handler({ method: "GET", query: { school: "school-a" } }, res);
    ...
    assert.doesNotMatch(
      ...
      /private@example|connection string|database details|detail/,
    );
  }
});
