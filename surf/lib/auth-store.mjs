...
export const sessionTokenHash = (token) =>
  crypto.createHash("sha256").update(token).digest("hex");
export const validSessionToken = (token) =>
    ...
    async find(token) {
      ...
      const rows = await sql`
        SELECT u.id, u.school_id, u.role, u.name, u.family_name, u.email, u.phone,
               ...
               COALESCE((SELECT json_agg(a ORDER BY lower(a.name),a.school_id) FROM account_school_access a WHERE a.user_id=u.id),'[]'::json) AS school_access
