...
import { isPlatformAdmin, schoolAccess, withSchool } from "./school-access.mjs";
...
async function userSessionPayload(user) {
  ...
  const authority = { authority: user.authority, platform: user.platform };
  const access = user.school_access || [];
  const role = authority.platform ? "platform_admin" : "student";
  const schools = access.map((a) => ({
    id: a.school_id,
    slug: a.slug,
    name: a.name,
    membershipId: a.membership_id,
    status: a.status,
    roles: a.status === "active" && a.school_open ? a.roles : [],
    owner: a.is_owner,
    open: a.school_open,
  }));
  ...
  const familyName =
    typeof user?.family_name === "string" ? user.family_name.trim() : "";
  const photoUrl =
...
export async function requireAuth(req, res, options = {}) {
  ...
  let scoped = session;
  if (options.schoolId) {
    // Check current school lifecycle even for platform administrators.
    const [school] =
      await sql`SELECT id FROM schools WHERE id=${options.schoolId}::uuid AND deleted_at IS NULL AND workspace_status NOT IN ('closed','suspended')`;
    if (!school || !schoolAccess(session, options.schoolId).viewSchool) {
      res.status(403).json({ ok: false, error: "Forbidden for this school" });
      return null;
    }
    scoped = withSchool(session, options.schoolId);
  }
  if (options.roles?.length && !isPlatformAdmin(session)) {
    const permitted = options.schoolId
      ? options.roles.some(
          (role) =>
            role === "student" ||
            (role === "school_admin" && scoped.capabilities.manageSchool) ||
            (role === "coach" && scoped.capabilities.teach),
        )
      : options.roles.includes("student") ||
        session.schools.some(
          (s) =>
            s.open &&
            s.status === "active" &&
            (options.roles.some((role) => s.roles.includes(role)) ||
              (s.owner && options.roles.includes("school_admin"))),
        );
    if (!permitted) {
  ...
  if (
    ...
    scoped.role === "student" &&
    normalizeEmail(options.studentEmail) !== normalizeEmail(session.email)
  ) {
